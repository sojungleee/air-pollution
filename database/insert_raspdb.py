...
def insert_raspdb_air_quality(geohash: str, device_id: str, ozone:float, co: float, pm10: int, pm25: int, timestamp: str):
    # connect to database
    db = pymysql.connect(host='localhost', user='root', password='raspberry', 
    db = 'raspdb', charset='utf8')
    cur = db.cursor()
    ...
    # insert
    sql = "INSERT INTO air_quality_sensor(geohash,receive_time,device_id,ozone,co,pm10,pm25) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    val = (geohash, timestamp, device_id,ozone, co, pm10, pm25)
    cur.execute(sql, val)
